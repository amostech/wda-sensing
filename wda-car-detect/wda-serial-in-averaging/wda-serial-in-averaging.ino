...
void MedianAD(float arr[], float* mad /*where to store mad*/, float* median /*where to store the median*/) {
  ...
  int i = 0;
  float deviations[AVERAGING_BUFFER_SIZE];
  memset(deviations, 0x00, sizeof(float)*AVERAGING_BUFFER_SIZE);
...
void calculateMagneticEnvironmentalAverage() {
    ...
    float avg_buff[AVERAGING_BUFFER_SIZE];
    memset(avg_buff, 0x00, sizeof(float)*AVERAGING_BUFFER_SIZE);
...
void loop()
{
  ...
  if( incomingLine.equals("AVG_BUFFSIZE_SET") ) {
          incomingLine = SerialUSB.readStringUntil('\n');
          incomingLine.toCharArray(charArr,1024);
          if(sscanf(charArr, "%d", &AVERAGING_BUFFER_SIZE) == 1) {
            SerialUSB.print("AVG_BUFFSIZE_ACK:");
            SerialUSB.println(AVERAGING_BUFFER_SIZE);
            canStart = true;
        } 
  } else if( incomingLine.equals("GET_FINALAVG") ) {
        SerialUSB.print("FINALAVG_ACK,");
        SerialUSB.print(AVERAGING_BUFFER_SIZE);
        SerialUSB.print(",");
        SerialUSB.print(gMagneticOffset);
        SerialUSB.print(",");
        SerialUSB.println(gMAD);
      ...
      // Run average only once for that particular BUFFER SIZE!!
